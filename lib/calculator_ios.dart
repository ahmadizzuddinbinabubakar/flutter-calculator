...
//todo: handle big numbers. either use e^ or make number size smaller
class CalculatorIOSApp extends StatelessWidget {
    ...
    switch (operation) {
      case UnaryOperation.changeSign:
        if (result != null) {
          ...
          if (operand2 != null) {
            operand2 = (-double.parse(operand2)).toString();

            if (operand2.toString().endsWith(".0")) {
              operand2 = int.parse(operand2.toString().replaceAll(".0", ""));
            }
            display = operand2;
          }
        } else {
          if (operand1 != null) {
            operand1 = (-double.parse(operand1)).toString();

            if (operand1.toString().endsWith(".0")) {
              operand1 = int.parse(operand1.toString().replaceAll(".0", ""));
            }
            display = operand1;
          }
        }
